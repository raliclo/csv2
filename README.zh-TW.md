...
**一個位址只跟它來源的那個檔案一樣新，而它有一種「會錯、卻沒有任何東西會說」的可能。**
若檔案旁邊有一份 `.index` sidecar，而該檔案後來被改成 O(1) 戳記看不出來的樣子——大小相同、
mtime 相同、首尾位元組也相同——搜尋就會信任它，回報的紀錄號可能是偏掉的。上面那三行於是
全部以 0 結束，而那次編輯落在隔壁那一筆上。兩個都很便宜的做法可以讓它不可能發生：
...
**一次寫入會順手修好 sidecar，而那會抹掉證據。** 若一份過期的索引把一次編輯送到錯的那一筆，
那次編輯會重寫整個檔案、並在過程中建出一份正確的 sidecar——於是事後的 `--verify-index` 會說
`index OK`、以 0 結束,而它說的是你剛剛弄壞的那個檔案。兩半各自都有記載,咬人的是它們的組合：
請在編輯「之前」驗證，不是之後。
