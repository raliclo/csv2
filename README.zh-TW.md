...
保護
  ...
  --                    「下一個」引數是資料，不是旗標：以減號開頭的值寫成
                        `-update 1:2 -- --in-place`。它不是「其後一律視為資料」
...
索引
  ...
  --build-index         立即建立 sidecar，任何檔案大小都會建。否則索引只會以「副作用」
                        ...
                        建一個、-tail 因為本來就要讀完整檔而建一個——只用 -mid 的話永遠
                        不會有；小檔案除非你開口要，否則沒有任何東西會替它建；而
                        `-append --in-place` 也不會建，因為它的快路徑根本不會把檔案讀到
                        結尾。既有的索引則「會」被那次追加延續
...
那三行於是全部以 0 結束，而那次編輯落在隔壁那一筆上。兩個都很便宜的做法可以讓它不可能發生：
...
讀到結尾也會建。`-mid`、`-contains`、`-r`、`-get` 永遠不會建。`-append --in-place` 不會建
——它的快路徑根本不讀到結尾——但它「會」延續一份已經存在的。
...
- **索引永遠是最佳化，永遠不是必要條件。** 沒有索引時行為完全相同。過期、截斷、
  ...
  留在原地，每次讀取時被丟棄一次。`-append --in-place` 同樣不會替換它，因為它的快路徑根本
  不會讀到檔尾；它只延續一份「本來就有效」的索引，對受損的那一份則不動。單純的 `-contains`
  或 `-r` 會讀過
