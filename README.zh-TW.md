...
`csv2 -head 1 --json -i f.csv | head -1`。沒有「只回傳標頭」的動詞。
...
以 `--md-table N` 從一份文件中選出一張表時，每一行 `--json` 紀錄上的 `line` 是相對於
**那張表**、而不是相對於文件的：無論那張表位於檔案多下面，第一筆資料列都是 `line 2`。
要把它們對回文件的腳本，必須自己加上那張表的偏移量，而 csv2 不回報那個數字。
...
目的地是 Markdown 的編輯**必須**給 `-md`，那不是選配。編輯一個 `.md` 檔而不給它會被拒絕、
