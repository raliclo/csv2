...
**唯一的例外是 `--headers 1|2`。** 它在 `-si` 時為必要，但**同樣可與 `-i` 併用，且會覆蓋
副檔名**。該覆蓋不會與檔名交叉檢查，因此 `--headers 1 -i a.csv2` 會被接受，並以「猜錯」
的方式解析該檔案。

後果比「讀錯」更嚴重，因為它會被寫回去。以 `compare/vs-sqlite.csv2` 實測（2026-08-16）：
...
22 筆變成 21 筆：中文標題列消失，一筆**資料**列取而代之成為第二列標頭——而該檔案在結構上
仍然合法、讀取時也不會報錯。同一個覆蓋也會改到錯的儲存格：`--headers 2 -update 1:1 X
-i a.csv` 動到的是第 2 筆而非第 1 筆，且以 0 結束。

**它還會使本文件所推薦的檢查失效。** 下方把 `--json` 的 metadata 行當成「斷言解析結果符合
預期」的手段——但 `--headers` 改變的正是你要拿去斷言的那個數字，於是該行自相矛盾：
`{"meta":{"format":"csv2","headers":1,…}}`。一份只有一列標頭的 `.csv2` 並不存在。
**斷言通過了，而解析是錯的。**

只有在副檔名確實標錯時才對 `-i` 使用 `--headers`，且絕不要與編輯動詞併用。
