...
| 其他（`.txt`、`.log`…）或沒有副檔名 | 0 列 | 每行一欄；位元組原樣保留 |
...
meta 行的 `fields` 是「每筆有幾個欄位」：三欄的 CSV 是 3，而 `.lines` 檔是 0，因為那裡的欄數不固定。
...
`.md` 都指名不到它。它也是唯一一個「會宣告的副檔名蓋不過去」的值。一個會宣告的副檔名拒絕的是**不一致**：
`.csv2` 搭配 `--headers 1`、`.csv` 搭配 `--headers 2`，兩者都是錯誤；而一個**與副檔名一致**的
`--headers` 會被接受，且什麼也不做。`0` 則無論副檔名說什麼都會被拒，因為它是**拒絕回答**那個問題，
不是換一個答案。直到 2026-09-07 之前，這裡寫的是「`.csv2` 搭配 `--headers 1|2` 會被拒」，那建立了
一個並不存在的乾淨二分法——而底下那個展示「不一致」情況的錯誤範例，自始至終都是對的。
