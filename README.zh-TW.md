...
- **索引永遠是最佳化，永遠不是必要條件。** 沒有索引時行為完全相同。過期、截斷、
  ...
  比沒有索引糟得多。**在 2026-08-18 之前，這句話對「索引自身的內容」並不成立**：所有檢查
  描述的都是資料檔——大小、mtime、首尾位元組的雜湊、項目數——沒有任何東西描述索引，因此
  偏移量裡一個翻轉的位元通過了全部檢查，`-mid 1,1` 回傳了一段從欄位中間開始的碎片，以
  「一筆紀錄」呈現，rc=0。檔頭現在帶著一個涵蓋整份索引（含偏移量）的檢查碼。損毀的索引會被
  丟棄，而下一個讀完整個檔案的操作會寫回一份好的。由 T68 斷言。
