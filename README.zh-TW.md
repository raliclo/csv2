...
選取
  ...
  -mid a,b              第 a 到第 b 筆，含兩端；`a,` 與 `,b` 為開放端。
                        ...
                        會在 rc=0 下輸出空的——與「一個確實存在、而且是空的視窗」
                        無法區分。要分辨它們，請讀 --json 結尾那行 meta 的
                        `records`：那是實際讀到的筆數，因此 `records < a` 就代表
                        那個視窗從未被抵達。-head 與 -tail 以同樣方式截斷
  -t                    輸出帶上標頭列（預設不帶）
...
這兩個迴圈**什麼都不會印**——csv2 在正常路徑上不輸出任何東西——因此那些標記必須事後從檔案裡
讀出來：
