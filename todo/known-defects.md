...
對相同資料列各執行五次並取最佳值：`.csv` 為 0.551 秒，`.csv2` 為 0.925 秒，
因此 `.csv2/.csv = 1.68x`。完整輸出見 [`verifications/measure_output.txt`](../verifications/measure_output.txt)。
這再次確認 JO 仍是效能限制，不是正確性錯誤；本次沒有宣稱已最佳化。
