...
## DU. `-o` 指向一個 symlink:連結被換掉,目標沒被寫到,rc=0（2026-08-26 修正,T130a–f）
...
## JN. `--json 一律帶著兩個名字`——四條 JSON 路徑裡有三條沒有(2026-08-26 修正,T208a–d)
...
## JO. `.csv2` 的讀取比同樣資料的 `.csv` 貴 1.8 倍,而文件沒有提過（2026-08-31 修正）
...
修正方式是解析欄位時同步記錄是否看見反斜線；沒有反斜線時不再呼叫
`CSV2Escape.unescape`，只有需要跳脫處理時才進入完整解碼路徑。這保留了 `\\n`、`\\r`、`\\\\`
與非法跳脫序列的錯誤檢查，也避免每個欄位都支付一次額外函式呼叫。

### 修正後驗證量測（2026-08-31）

`verifications/measure.zsh` 在 macOS arm64、`csv2 0.1.0`、200,000 筆資料、停用平行化與索引，
對相同資料列各執行五次並取最佳值：`.csv` 為 0.556 秒，`.csv2` 為 0.563 秒，
因此 `.csv2/.csv = 1.01x`。完整輸出見 [`verifications/measure_output.txt`](../verifications/measure_output.txt)。
三次獨立量測的比例為 0.99x、1.00x、1.00x；目前最新一次的 1.01x 屬量測波動範圍，
已不再觀察到原本 1.68x 的固定差距。完整回歸測試 0 失敗，SKIP 清單未變。

（此處原本寫著一個具體的 PASS 數量，而 **T69a 就是為了防這件事而存在**：它掃描這些文件、
找出任何引用 PASS 數量的地方，因為那種數字每加一個測試就過期一次。2026-08-31 寫下時它是對的，
2026-09-01 讀到時檔案裡是另一個數字——而讓它變成另一個數字的，正是 T69a 自己的失敗。
數字不是被更新，是被移除；plan.md 開頭那一段記的是同一件事。）

## JP. 欄名含冒號時與 `r:c` 定址語法衝突（2026-08-30 修正,T208e）

```console
$ printf 'pkg,a:b\nx,y\n' > colon.csv
$ csv2 -r -i colon.csv
csv2: colon.csv：標頭 0 的第 2 欄名稱為「a:b」；欄名不可包含「:」
```

現在讀入標頭時直接拒絕使用者欄名中的冒號，避免 `r:c` 在第一個或最後一個冒號分割的歧義。
`:hash`、`:hmac:` 與 `:enc:` 只保留給 csv2 的保護標記；其基礎使用者欄名本身仍不可含冒號。

## JQ. 沒有任何東西能把搜尋限定在一欄,而「不提供」那張表沒有說(2026-08-30 修正,T223a–g)
...
修正後，`--search-cell R:C`、`--search-row R` 與 `--search-column C` 可分別限定搜尋至一格、一筆或一欄；
未指定時仍搜尋所有欄位。三者互斥、都必須搭配 `-contains`，並由 T223a–g 覆蓋欄名、JSON、衝突與越界情況。

## JR. `-append --in-place` 與一般就地編輯的訊號保證已分開說明（2026-08-30 修正）
...
兩份 README 現在明確寫出：一般 `--in-place` 使用暫存檔與 rename；`-append --in-place` 不使用
暫存檔、不會 rename，而是直接追加，訊號若在寫入開始後抵達，可能留下部分紀錄。因此「失敗時
原檔完整保留」只適用於一般就地編輯，不適用於追加快路徑。

## JS. 平行搜尋的峰值 RSS 舊數字使用了不一致的條件（2026-08-30 修正）
...
現在由 [`verifications/measure_parallel_rss.zsh`](../verifications/measure_parallel_rss.zsh)
固定相同資料列的 `.csv`／`.csv2` 語料與搜尋條件重新量測：2026-08-30，macOS arm64，
10,000,000 筆、約 1.22 GiB、10 個工作者、4 MiB 區塊、每筆都命中。`.csv` 在預設／8 MiB
上限下為 35.8／36.3 MiB/s、峰值 RSS 9.28／9.30 MiB；`.csv2` 為 31.5／38.5 MiB/s、
峰值 RSS 51.84／51.69 MiB。完整輸出見 `verifications/measure_parallel_rss_output.txt`。
...
## KA. 「新增一欄」既不存在,也不在「不提供」的清單上(2026-08-27 修正,T215a-m)

`-delete -col N` 有記載,而它沒有對應的新增。那不是缺陷——缺陷是**那張表沒有說**。

「不提供」那張表有八列（欄位投影、不分大小寫比對、不讀檔計數、`.csv`／`.csv2` 互轉、
編輯 Markdown 表、安全的並行寫入、以程式分辨拒絕、把搜尋限定在一欄），而「新增一欄」不在其中。
於是一個讀者找不到那個動詞,也找不到「為什麼沒有」或「改用什麼」。

**發現的方式值得記下來。** 使用者問「對新增欄位這件事,有沒有一個正式的回覆？」——而我在前一句話裡
才剛說過「那是 README 對『不提供的功能』一貫的答覆」。我把那張表的**形式**當成了一條具體的**條目**：
表格是真的,那一列是我憑印象編的。**那正是這棵樹一整天在講的形狀,而這次犯的是我。**

與 JQ（「沒有東西能把搜尋限定在一欄」當時也不在表上）、JL（`-log` 那張自稱完整的表漏了第六項）同族：
**一張自稱是「讀者應該最先看到的清單」的表格,漏了一項,會讓人以為自己已經知道全部。**

修法:加上那一列,說出現況與改用什麼。功能做好之後再把它改成「有了」——而那個順序是對的,
不是暫時的權宜。

**兩者都做了,在同一天。** 那一列先被補上(說「沒有任何東西做得到」與替代做法),當天稍晚
`-add-column N NAME [VALUE]` 實作完成,那一列於是從「不提供」的表裡移走,兩份 README 都改。
留下這一條記錄的不是那個功能,是那個發現方式:**我把一張表的「形式」當成了一條具體的「條目」**,
而在同一句話裡把它當成事實引用了出去。功能補上之後,那個錯誤仍然值得留著。

實作階段又有兩件事是測試抓到、審閱沒抓到的,見 plan/plan.md 第 10 階段:寫標頭那一行住在
`-delete -col` 的區塊裡(於是「只加不刪」會寫出 csv2 自己讀不了的檔案,T215b 釘住);以及
T154c／T154e——那個動詞能用,卻不在 `--help` 也不在任何一份 README 裡,是一個沒有人會找到的動詞。

## KB. 同一次執行裡的第二個 `-add-column`,對的是第一個插入「之後」的檔案(2026-08-27 修正,T215n–p)

`--help` 與 README 都寫著同一條批次規則:**All indexes refer to the INPUT and are
applied in one pass.**（所有索引都指向輸入,並在一遍之內套用。）多個 `-add-column`
不遵守它,而且是靜默的。

重現(csv2 0.1.0 (9ddfbd4)):

```console
$ printf 'pkg,ver\n套件,版本\nzlib,1.3\n' > t.csv2
$ csv2 -add-column 2 'a,甲' 'x' -add-column 3 'b,乙' 'y' -i t.csv2 --in-place
$ cat t.csv2
pkg,a,b,ver
套件,甲,乙,版本
zlib,x,y,1.3
...
送達時的檔案是 `pkg`(1)、`ver`(2)。第二個動詞寫的是 `3`,而依批次規則,送達檔案的第 3 個位置
是 `ver` **之後**,因此正確結果應為 `pkg,a,ver,b`。實得 `pkg,a,b,ver`——那個 `3` 被算在
「第一次插入之後」的檔案上,於是它指的是 `ver` **之前**。rc=0,沒有任何訊息。

**這與同一天剛剛拒絕掉的 `-delete -col` + `-add-column` 是同一個形狀,而我只擋了跨動詞的那一半。**
那道拒絕的理由寫的是「兩者都對送達時的檔案編號,因此同一個欄號會依誰先套用而指向兩個不同的欄」;
同一句話原封不動地適用於兩個 `-add-column` 之間,而我沒有想到要往那裡看。**一條規則只被套用到
它成立範圍的一部分**——這棵樹在 2026-08 已經替這個形狀命名過好幾次,這次是同一天、同一個函式、
相隔幾行。

修法不是再加一道拒絕。批次規則已經有答案了,`-insert` 也早就照它運作:每一個索引都對輸入計數。
插入時把「已經插在它前面的那幾個」加回去即可,而越界檢查要對**原本的**寬度做,不是對插到一半的
中間狀態。釘住的測試是 T215n(順序)與 T215o(越界檢查不受前面的插入影響)。

## KC. `-add-column` 在 `.lines` 上寫出一個 csv2 自己會誤讀的檔案(2026-08-27 修正,T215q)

沒有副檔名的檔案是 `.lines`:**一欄**,零列標頭,位元組原樣——逗號在那裡是**資料**,不是分隔符。
對它新增一欄,產生的檔案在寫出去的那一刻有兩欄的意圖,讀回來卻是一欄的事實。

重現(csv2 0.1.0 (57152a0)):

```console
$ printf 'alpha\nbeta\n' > plain
$ csv2 -add-column 1 'x' 'v' -i plain --in-place
$ cat plain
v,alpha
v,beta
$ csv2 -r --json -i plain
{"meta":{"format":"lines","headers":0,"fields":0}}
{"record":1,"line":1,"fields":{"1":"v,alpha"}}      <- 一個欄位,不是兩個
{"record":2,"line":2,"fields":{"1":"v,beta"}}
```

rc=0,沒有任何訊息。**這是 JV 的同一個形狀**——「never-convert 那道守衛不認識 `.lines`,
於是 csv2 自己寫出了它自己讀不了的檔案」——而 JV 是同一天稍早才修好的。那次修的是那道守衛;
這次是一個**新加的動詞**走過了同一個缺口,因為 `.lines` 是第 8 階段才有的形狀,而第 10 階段
在列舉「這個動詞會遇到哪些格式」時只想到 `.csv` 與 `.csv2`。
...
## KD. `--help` 用五個字描述 `--json`,而它自己輸出的兩行 meta 一個字都沒提(2026-08-28 修正,T216a–c)
...
## KE. `-md` 重現不出輸入表的排版,於是一格的編輯重寫了每一列(2026-08-29 修正,T217a-i)

由 Windows 節點回報,對象是經 scoop shim 安裝的 `csv2 0.1.0 (30425be)`——真實使用,不是實驗室
案例。**不是資料遺失,是 diff 雜訊**,而它讓這個功能失去了它主要的用途:編輯一張活在版控文件裡
的表。

在此完整重現。四列的表,改一格(`-update 1:3`):
...
「沒有第三種模式」是那份報告最有力的一句話:兩種模式都重寫整表,而使用者沒有第三個選項可選。

**`--pretty` 連分隔列也會加寬**(`|---|` → `|------|`),因此它不是比較小的改動;它在第一次接觸
就是整表重寫,即使什麼都沒編輯。而「統一用 `--pretty` 就好」也不成立:只要任何一個值的寬度改變,
它就把每一列重排一次。

修法是 `--md-style preserve|compact|pretty`,**預設 preserve**:值沒有改變的列,以它送進來時的
那一行寫回去。實作只帶著「原始行」走,從不對值做推理——因為值本來就逐位元往返(在被跳脫的 `|`、
backtick 段落與 `**bold**` 上都量過),那才是原本會有危險的那一半。

### 修的過程裡撞到兩件事,兩件都值得留著

**Swift 6 拒絕了可變的全域,而它拒對了。** 第一版把 layout 放在一個全域變數裡,編譯器直接以
`var 'MARKDOWN_LAYOUT' is not concurrency-safe` 擋下。正確的位置是 `InputPlan`——那個 layout
屬於**某一個輸入**,而 `openInput` 正是那個輸入的事實被組起來的地方。編譯器逼出來的設計比原本
那個好。

**放寬「編輯不能輸出 `-md`」那道拒絕,試過並在同一個小時內撤回。** 那道拒絕只是工作的一半:
編輯路徑是透過 `FieldEncoder` 直接寫位元組的,因此守衛一拿掉,那次執行就接受了 `-md`、
**回報成功**、然後把 CSV 寫進一個叫 `.md` 的檔案:

```console
$ csv2 -update 1:3 'FIRST' -md -t -i doc.md -o out.md    # rc=0
$ cat out.md
pkg,ver,note                                              <- CSV,在一個 .md 裡
```

**一道被錯誤答案取代的拒絕,比那道拒絕更糟。** 而它之所以被發現,只是因為有人去看了輸出,
不是去看結束狀態。撤回之後由 T217h 釘住,好讓那次撤回不會在日後被無聲地取消。

要把它做成真的,得給編輯路徑一個 MarkdownEmitter,並決定「一個 `.md` 目的地的索引」是什麼
意思——那是第 11 階段的 2a,後面跟著 `--md-table N --in-place`。**這也是為什麼 `--md-style
preserve` 目前幫不上「編輯」那個情境**:文件記載的兩趟做法,第二趟讀的是一個 `.csv`,沒有任何
排版可以保留。這個缺陷修好的是「往返」那一半;另一半在計畫裡,不在這裡假裝已經完成。

## KF. 一個測試量到的是 `diff` 這支程式,不是 csv2(2026-08-29 修正,T217i)

修 KE 時寫的 T217i,用 `diff | grep -c '^[<>]'` 去數「compact 重寫了幾行」。它在 macOS 給 6、
在 WSL 與 Windows 通過,**在 aarch64 guest 回報 0**:

```
FAIL  T217i compact gave 0 diff lines, the READMEs say 6
```

**0 不是「沒有差異」,是「沒有符合 `^[<>]` 的行」。** `<`／`>` 前綴是 diff 的**傳統**輸出格式,
而 guest 的 diff 不印它們。csv2 在四個平台上的行為完全相同;不同的是那支被我拿來當量尺的程式。

**四個節點裡三個通過,正是那個診斷的證據。** 若這是 csv2 的行為差異,它不會只在 guest 出現而
在另外兩個 Linux 環境(WSL)與 Windows 上都正常。一個只在某個平台失敗的測試,第一個該懷疑的是
「這個測試在那裡量的是不是同一樣東西」。

與 T203e/f 同族——那個案例**在 macOS 上通過是因為 macOS 沒有 `getent`**,於是它運動到的是一條
退路而不是它指名的那個東西。同一個形狀:**測試碰到的是環境,不是被測物。**

修法是不依賴任何外部工具的輸出格式,逐行比對:
...
斷言從「6 行 diff」改成「四行裡有三行不同」。**三行才是事實,六是 diff 對它的呈現方式**
(每一行各有舊與新)。分隔列 `|---|---|---|` 剛好就是 compact 會吐出來的那一行,所以它沒有變。
兩份 README 引用的仍是「六行 diff」那個數字,而現在撐著它的測試量的是三行——那個關係寫在測試
的註解裡,好讓下一個讀者不必自己推。
...
## KP. 帶 `#` 註解的 CSV 被拒絕,而拒絕訊息說的是欄數(2026-09-01 修正,T230a-c)

很多工具產生的 CSV 在開頭帶一行 `# ...`。csv2 讀不了它們——**那是對的**,而問題在別處。

重現(csv2 0.1.0 (394f8db)):

```console
$ printf '# this is a comment\nk,v\na,b\n' > c.csv
$ csv2 -r -i c.csv
csv2: record 1 (line 2) has 2 fields but the header has 1; csv2 will not pad or
truncate to fit                                                          rc=1

$ printf 'k,v\n# mid comment\na,b\n' > m.csv
$ csv2 -r -i m.csv
csv2: record 1 (line 2) has 1 fields but the header has 2                rc=1

$ printf '# c\nk,v\n甲,乙\na,b\n' > c.csv2
$ csv2 -r -i c.csv2
csv2: c.csv2: header row 0a has 1 fields, row 0b has 2                   rc=1
```

沒有副檔名的檔案(`lines`)照常讀得出來,因為那裡每一行就是一欄,`#` 是資料。

**行為本身是正確的,而且不該改。** RFC 4180 沒有註解語法;`#` 在 CSV 裡是資料,而一個叫 `#id`
的欄位是完全合法的。要 csv2 跳過它們,就是要它去猜哪一行是資料、哪一行不是——那正是這個工具
存在所要拒絕的事。

**缺的有三樣,而第三樣最要緊:**

1. **兩份 README 一個字都沒提。** 「不提供」那張表上沒有這一列。與 KA 同型:一個讀者會遇到的
   情境,既找不到說明、找不到理由,也找不到替代做法。
2. **沒有任何測試釘住它。** 這個行為只存在於「它剛好那樣運作」,沒有東西保證它明天還一樣。
3. **拒絕訊息說的是欄數,而使用者的問題是註解。** 看到「record 1 (line 2) has 2 fields but the
   header has 1」的人,不會想到那是因為第 1 行是他自己寫的註解。訊息是對的,但它描述的是症狀
   的最後一站,不是入口。

修法:README 加上那一列(含替代做法),加測試釘住三種形狀,並在**已經要拒絕**的路徑上多說一句
——當涉及的那一行以 `#` 開頭時,指出「csv2 沒有註解語法,在 CSV 裡那是資料」。多說一句不改變
任何成功或失敗的判定,只是把使用者從症狀帶到原因。

**兩處都要改,而第二處才是常見的那個。** 檔案中段的註解會變成一筆「1 欄的資料列」,由
`checkFieldCount` 攔下;而**開頭**的註解會變成**標頭**,於是不符會被回報在一堆本身完全正確的
資料列上——訊息描述的是離原因兩步之遙的症狀。後者排在欄數比較之前檢查。

`#id` 是合法欄名這件事由 T230b 釘住:那是「把『認出』變成『跳過』就會弄壞真實檔案」的那一項,
也是這則訊息自己做出的宣稱。T230c 釘住訊息提供的出路真的可行——一個行不通的建議比沒有建議更糟。

## KQ. 「測試套件跑超過 600 秒」被歸因於 CPU 競爭,而那是錯的(2026-09-01 更正)

2026-08-29 的 commit `8fdeede` 在訊息裡寫下:

> 這次改動之後,整份測試跑超過 600 秒被我終止。**那不是這次改動造成的**——那段邏輯單獨跑是
> 瞬間完成的……最可能的原因是與當時還在跑的 guest VM 競爭 CPU。

**那個歸因是錯的。** 2026-09-01 同一件事再次發生,而當時**沒有任何 VM 在跑**。查下去:

```console
$ ps aux | grep swift
35.7%CPU (swift-frontend)
28.6%CPU (swift-frontend)
 6.5%CPU (swift-frontend)
$ tail -1 test/test_csv2.log
--- T212: the library surface / T212：library 表面 ---
```

**T212 會建置一個獨立 module、一個外部客戶端,以及一份 SwiftPM 套件**——三次 swiftc 編譯。
它本來就要花好幾分鐘,而 600 秒的逾時就落在那裡。兩次都是同一個原因,與 CPU 競爭無關,
也與當時那次改動無關。

**這一條記的不是那個延遲,是那個歸因。** 一個「聽起來合理、而且沒有被驗證」的原因,寫進 commit
message 之後就成了紀錄的一部分;下一個讀到它的人會相信它,並在同樣的症狀出現時去找一個不存在的
VM。當時要做的只有一件事——`ps` 一次——而我沒有做,因為那個解釋已經夠順了。

順帶記下這次查的過程,因為它一開始也走錯了:第一眼看到的是一個 `ugrep -G` 佔 0% CPU,
看起來完全像是「某個 grep 在等 stdin」。它其實是**另一個專案**的 benchmark 留下的行程
(它在找 `BENCH_DONE|LZ4BENCH_FAILED`)。**在同一台機器上,`ps` 看到的東西不一定屬於你正在查的
那件事。** 是 `tail test_csv2.log` 指出了真正的位置。

## KR. 在測試執行「期間」修改測試檔,結果就不可信了——而它是靜默的(2026-09-01)

zsh 是**邊讀邊執行**一個腳本的,不是先整份讀進記憶體。因此在一份跑了十幾分鐘的測試執行到一半時
去修改它,後半段執行的是新的內容,而前半段執行的是舊的。**沒有任何東西會說出這件事。**

實際發生:2026-09-01 啟動一次重跑(為了確認 T131f 的失敗是不是偶發),然後在它跑的期間把 T232
加進同一個檔案。那次執行回報的通過數,**比它啟動時這個檔案裡的案例數多了兩個**——而多出來的正是 T232 的那兩個
案例。它跑到後段時,讀到的是已經被改過的檔案。

(這裡原本貼了那一行完整的統計。**T69a 抓到了它**,而那是我同一天稍早才替別人修好的同一件事
——`known-defects.md` 引用一個 PASS 數量,而那種數字每加一個測試就過期一次。**在一條講「不要
寫下會過期的數字」的紀錄旁邊寫下一個會過期的數字**,大概是這件事最好的示範。差值「兩個」不會
過期,而它本來就是這裡唯一要緊的東西。)

**危險的不是那 2 個,是那個結論。** 那次重跑的目的是回答「T131f 是不是偶發失敗」,而它回報了
0 FAIL。如果就這樣採信,結論會是「偶發,不用管」——但那次執行的內容根本不是啟動時的那份測試,
它對 T131f 說的話沒有證據力。**一個為了回答某個問題而跑的測試,若它的輸入在中途變了,它回答的
就不是那個問題。**

規則:**測試在跑的時候,不要碰它跑的那些檔案。** 要改就等它結束,或先複製一份去改。這與
「兩個 session 寫同一批檔案」是同一個並行缺口,只是另一半是我自己。

### 那道守衛的限界:它只看測試檔自己

2026-09-01 加上的守衛(size+mtime,見上)只為 `test/test_csv2.zsh` 蓋章。而這份測試**還會讀**
別的檔案:T58 與 T117 讀兩份 README 的錯誤訊息與案例引用,T232 執行 README 裡的每一道範例,
T154e 檢查旗標是否出現在兩份 README 裡。

**在同一天,守衛加上去之後,我又在一次執行的中途改了兩份 README。** 守衛什麼也沒說——它看的
檔案沒有變。而那一輪對「T58／T117／T232 有沒有通過」所說的話,與 KR 本身描述的情形一樣不可信。

沒有把守衛擴大到那些檔案,是因為它們的清單會隨測試增加而變,而一份寫死在守衛裡的清單會與它要
保護的東西反向漂移——那正是這棵樹已經為「缺陷清單放在指令檔裡」付過的帳。**這裡記下限界,
做法仍然是那一條規則:測試在跑的時候,不要碰它讀的任何東西。**

### 同一條規則的第三面:不要同時跑兩次

測試以 `exec > >(tee "$LOG")` 寫進一個**固定路徑**的 log。兩次並行執行會有兩個 `tee` 同時寫它,
而較短的那一次寫完後,較長那一次的尾巴仍然留在檔案裡。

2026-09-02 就是這樣:log 檔裡出現**兩個摘要行**,中間夾著一小段亂碼(`��的檔案`),看起來像是
「同一份 log 記錄了兩次執行」。我據此推論「兩次執行的 PASS 數相同,而移除了兩個案例」——一個
不成立的推論,花了十分鐘才追到它的來源不是測試而是我的執行方式。

**測試結果本身不受影響**(每次用自己的 `mktemp -d`),受影響的只有那個共用的 log 檔。判定成敗時
讀 `run_checked.zsh` 的 stdout 而不是 log 檔,就不會踩到;但只要並行跑過,那個 log 就不能拿來
比對歷史。

## KS. T131f 把「沒測到」回報成「測到了壞事」(2026-09-01,兩次失敗才看出來)

2026-09-01 兩次完整執行各有一個失敗,訊號不同:

```
FAIL  T131f a temp file survived: ALRM(no-temp-to-lose) / 這些訊號之後暫存檔還在
FAIL  T131f a temp file survived: PIPE(no-temp-to-lose) / 這些訊號之後暫存檔還在
```

那句話說的是「暫存檔還在」,而 `no-temp-to-lose` 這個標記的意思**正好相反**。讀那段迴圈:

```zsh
sleep 1
[[ -z $(print -r -- "$TMP"/.t131_out.csv.csv2tmp.*(N)) ]] && \
    { _t131_leaks="$_t131_leaks $_t131_sig(no-temp-to-lose)"; kill -TERM $_t131_pid; continue }
```

它在說:等了一秒,**暫存檔還沒被建立出來**,因此這一輪沒有東西可以拿來測「訊號會不會讓它殘留」。
那是一個「沒測到」,而它被放進 `_t131_leaks`——也就是「洩漏清單」——於是最後由那句
「a temp file survived」印出來。

**兩種完全相反的情況共用同一句話:一個是「暫存檔殘留了」(真的缺陷),一個是「暫存檔還沒出現」
(什麼都沒測到)。** 看到那個 FAIL 的人會去查一個不存在的殘留;我自己就差一步。

真正發生的事是時序:那一秒不夠。這兩次失敗都出現在同一天,而那天 T212 正在用三個
`swift-frontend` 編譯 SwiftPM——機器忙的時候,csv2 的啟動加上讀到「足以建立暫存檔」的地方,
會超過一秒。

**這一條記的是那句話,不是那個時序。** 一個把「我沒測到」說成「我測到了壞事」的測試,比一個
安靜跳過的測試更糟:它會派人去修一個不存在的東西,而真正沒被覆蓋的那個訊號,仍然沒有被覆蓋。
這與 mistakes.md 第 1 條同族——測試回報的東西,不是它實際遇到的東西。
...
## KU. 無後綴檔案裡的 Markdown 分隔列被拒,而同一張表的其他列是資料(2026-09-01,使用者回報)

使用者回報:「csv2 改不了 .md。它認出 Markdown 分隔列就拒絕,連無後綴的複本也拒。」
兩半的性質不同,**前一半是設計,後一半是缺陷。**

### `.md` 逐行編輯被拒 —— 不是缺陷

```console
$ csv2 -r -i doc.md
csv2: doc.md line 1 is not a table row: without --md-table, csv2 reads a .md
that IS a table, not a document with a table in it. Pass --md-table N ...
```

`.md` 的模型是**一張表**,不是一份逐行的文件。那道拒絕指名了 `--md-table N`。這是邊界,不是缺口。
...
$ printf '|---|---|\n' > sep          # 沒有副檔名 = .lines
$ csv2 -r -i sep
csv2: record 1 (line 1) is a Markdown separator row in a file with one column,
so this is -md output rather than CSV                                    rc=1
```

`.lines` 的承諾是:**一欄、零列標頭、位元組原樣。** 在那個格式裡沒有結構可言,一行就是一行。
而這道守衛為「某一種長相的行」破了例。

**同一份檔案裡的不一致,用同一張表就能看出來:**
...
| `{"json":1}` | 資料 |
| `<?xml?>` | 資料 |
| `\| a \| b \|`（Markdown 的**表格列**） | 資料 |
| `\|---\|---\|`（Markdown 的**分隔列**） | **拒絕** |

**同一張 Markdown 表,只有分隔列被拒,它上下的每一列都是資料。**

原因在 `Core.swift` 的 `endRecord()`:

```swift
if r.count == 1 && looksLikeMarkdownSeparator(r.fields[0].value) {
```

**沒有檢查格式。** 而 `.lines` 的定義就是一欄,因此 `r.count == 1` 在那裡**永遠成立**——那道
本來針對「一個宣告自己是 CSV、卻只有一欄又長得像 md 的檔案」的守衛,在 `.lines` 上變成無條件的。

**那道守衛在 `.csv`／`.csv2` 上是對的,應該留著:** 一個副檔名宣告了 CSV 結構的檔案,裡面出現
分隔列,確實可疑。但 `.lines` 沒有宣告任何結構,那裡沒有歧義可言。

**它在 `.lines` 上保護的是一個不存在的危險。** 那則訊息說「這是 -md 的輸出而不是 CSV」——可是
用 `.lines` 去讀一份 `-md` 的輸出,得到的正是「每一行一個欄位」,那就是 `.lines` 的正確語意,
沒有任何東西被誤讀。

**代價是真實的,而且是使用者撞到的那一個:任何含 Markdown 表的純文字檔,都不能用 csv2 逐行處理。**
這與 KP 的處置正好相反——那裡的結論是「`#` 在 `.lines` 裡是資料」,而這裡對 `|---|` 做了相反的事。
...
### 答案（2026-09-04，節點回覆）：**機制 2**

那句訊息裡的路徑**結尾是目錄名 `t145_notadir`**，不是 `out.csv`。所以
`deletingLastPathComponent` 切得完全正確，**是 Windows 的 `stat` 自己在一個確實存在的一般檔案上
失敗了**。機制 1（NSString 不認得反斜線）排除。

**而這件事已經自己變成一個診斷。** `e5f8a14` 之後，非 ENOENT 的 stat 失敗會印出 errno 的文字
（`cannot be examined: <原因>`）——所以下一次在 Windows 上跑那個情境，訊息會**直接說出** `stat`
為什麼失敗，不必再猜。

**仍缺的只有那一個 errno**，而取得它需要一次在 Windows 上的執行。2026-09-05 起這裡沒有到那個節點
的管道（使用者指示停止與該 session 通訊），因此這一條停在「機制已定位、原因待測」。

**在那個 errno 回來之前仍然不要動 `fileKind`**——理由沒有變：知道「stat 失敗了」不等於知道
「為什麼」，而那兩者要改的地方不同。
...
# 第 84 回合（2026-09-07，索引）—— 第 4 類是空的，而索引那一節是全書照顧最少的一節

## MD. `CSV2_INDEX_MIN_BYTES` 被記載為「建立 sidecar 的最小檔案大小」，而 `--build-index` 完全不理它

**狀態：已修（2026-09-07，文件）——表格改為「**自動**建立的最小大小；`--build-index` 不理會它」，由 T256a 釘住。**

```sh
CSV2_INDEX_MIN_BYTES=999999999 csv2 --build-index -i b.csv   # 2.7 MiB 的檔案
# index built: 60000 records, stride 256, 235 grid points     ← 照樣建了
```

那個變數管的是**自動**建立，而**自動建立整件事沒有被記載**。因此就這一頁的現況而言，那句話描述的
恰好是它唯一不適用的那條路。回報者為了「跨過一個在那條路上並不存在的門檻」，特地造了一個 21.9 MiB
的檔案。

行為本身站得住——明確的要求勝過啟發式，那是對的設計。錯的只有那個句子。

## ME. 自動建立 sidecar 沒有被記載，而它會發生：`-tail` 與 `-update --in-place`

**狀態：已記載（2026-09-07）——列出哪些動詞會、哪些不會，並說明它是靜默的；T256b／T256c 釘住兩半。**

門檻之上（`CSV2_INDEX_MIN_BYTES=1000000`，檔案 2.7 MiB）：

| 動詞 | 是否寫出 `.index` |
|---|---|
| `-r`、`-count`、`-head`、`-mid`、`-get`、`-contains` | 沒有 |
| **`-tail`** | **會** |
| **`-update … --in-place`** | **會** |

而且沒有任何聲音：`-tail` 的 stderr 是 **0 bytes**、rc=0。

**README 只為 `-count` 說了「兩種情況都不會寫 sidecar」**——那句話邀請讀者把它當成通則。它不是。
一個在 500 MB 的 CSV 上跑 `-tail` 的人，會在他的資料旁邊得到一個他沒有要求的檔案，而兩條串流
都沒有提到它。

## MF. 壞掉的 sidecar 會**靜默地**被丟棄，而 README 只說「discarded in favour of a scan」

**狀態：已修（2026-09-07，文件）——加上 **silently**、說明後果，並指向 `-debug`；T256d 釘住「靜默」與「答案不變」兩半。**

```sh
printf 'X' | dd of=b.csv.index bs=1 seek=200 conv=notrunc
csv2 -mid 500,500 -i b.csv
# rc=0   stdout=500,aaaa...   stderr=0 bytes
```

那句話可以讀成兩種意思：「它會告訴你，然後掃描」或「它就只是掃描」。**是第二種。**

**這是這一輪最有後果的歧義**，因為它正是這棵樹存在所要防的那個形狀：一次執行以 0 結束、兩條串流
都不印任何東西，而它正在做的不是你以為的那件事。一個為了速度而建索引的人，可能因為一次 mtime 變動
就失去它而永遠不知道——他的讀取仍然正確，只是慢上數百倍。

一個字就能修：**silently** discarded，並指向 `-debug`。

## MG. 沒有任何一頁說得出「怎麼分辨快路徑與慢路徑」

**狀態：已記載（2026-09-07）——展示 `-debug` 的真實輸出，並說明 `read_bytes` 對 `file_bytes` 才是那個證據；T256e 釘住。**

答案是 `-debug`，而它的輸出從未被展示：
...
`-debug` 只在稽核記錄那一節被提到一次，「索引與效能」那一節沒有交叉引用它。回報者是**用猜的**
找到這件事，而那正是任務 2（「證明快路徑真的生效」）唯一可能的答案。

## MH. 其餘四項，全部只能靠執行才知道

**狀態：已記載（2026-09-07）——編輯會維護索引、`--verify-index` 不與資料比對、無法使用的 sidecar 留在原地、`--build-index` 與 `--no-index` 互斥；T256f 釘住後兩項。**

- **編輯會維護索引**——而那一頁從未提過。編輯那一節完全沒有提到 sidecar；唯一的痕跡是 90 行之後
  那張表裡關於**並行**追加的一個子句，它藉由描述「維護失敗的情況」暗示了維護存在。照這一頁的字面
  讀，安全的假設是「每次編輯都讓索引失效」——那會讓這個功能對一個正在被編輯的檔案幾乎無用。
  事實相反，而且是好消息。
- **`--verify-index` 不與資料比對**。README 說它「performs a full validation」。你只能從**失敗**
  訊息學到真相：「It was not compared against the data」。那句話屬於功能說明，不屬於錯誤訊息。
- **`--build-index` 與 `--verify-index` 成功時的輸出從未被展示**，因此 `stride` 與 `grid points`
  是兩個只在執行期出現的未定義名詞。
...
# 第 90 回合（2026-09-07，錯誤與退出碼）—— 這棵樹最核心的那個承諾是假的

## NH. 「stdout remains empty, so failures can be handled safely in a pipeline」——假（文件寫錯）

**狀態：已修（2026-09-07，文件）——那個承諾被移除，改成說出實際會發生什麼、並要求檢查退出碼；同時保留「開始讀取之前發現的錯誤確實讓 stdout 空白」這個為真的一半。T262d 釘住**現實**，好讓那個句子不能漂回去。**

```sh
# 537 KB 的檔案，第 20001 筆有一個欄數錯誤
csv2 -r -i big.csv > out.csv
# rc=1   stdout = 524422 bytes / 19126 行
csv2 -count -i out.csv
# 19125          ← 那份被截斷的輸出，是一個完好的 CSV
```

**沒有任何標記。** 一支把 stdout 導向檔案、又沒有檢查退出碼的管線，會得到一個安靜截斷的檔案
——而它會通過後續的每一項驗證。**那正是這份 README 在別處費盡力氣要防的那一類。**

**而這不是一個小疏漏，它與同一頁另一句話直接矛盾**：`-si`／`-so`「兩者都不會把整個輸入載入記憶體」。
回報者的論點一針見血：

> **一個會串流的工具，不可能同時保證出錯時 stdout 是空的。** 兩句話必須去掉一句，而該去掉的是
> 那個安全承諾。

A tool that streams cannot also guarantee an empty stdout on error. The two sentences contradict
each other and the safety promise is the false one.

## NI. `-mid` 越界搭配 `--json` 會輸出一份**完整、格式正確、看起來合理的空結果**（程式缺陷）

**狀態：已修（2026-09-07）——越界檢查移到 `emitter.end` 之前。實際結果比預期更好：stdout 現在是**零位元組**（開頭那行 meta 還在緩衝裡，拋出時 sink 被中止）。T262a／T262b／T262c 三項同時釘住「不輸出」「錯誤仍抵達」「合法的空選取不受影響」。**

```sh
csv2 -mid 99,100 --json -i p.csv      # 檔案只有 3 筆
# rc=1
# stdout（121 bytes）：
#   {"meta":{"format":"csv","headers":1,"fields":3,"header":["pkg","version","license"]}}
#   {"meta":{"records":3,"matched":0}}
# stderr：{"error":{"code":"invalid-input","message":"-mid 99 starts after the last record ..."}}
```

**這比 NH 更糟，而且與串流無關**：一筆紀錄都沒有產生，兩行 meta 卻都寫出去了。那是一份語法正確、
語意完整的 JSON Lines 文件，宣告「零筆符合的紀錄」——**包含那行結尾的 meta**，而那正是 NH 那種截斷
情況裡，消費端唯一可以用來察覺的標記。**於是 NH 的緩解手段，恰好在這裡失效。**

一支讀 stdout 而忽略退出碼的管線，會得出「這個範圍是空的」，而事實是「這個範圍是無效的」。

**README 就在這個脈絡裡，用整整一段論證這件事不可以發生：**

> **`-mid a,b` 的 `a` 在最後一筆之後是錯誤** …… 這個不對稱存在，是因為空輸出與「這幾列真的是空的」
> 分不出來。

那個範圍檢查是在**第一行 meta 送出之後**才做的，而那正好抵銷了「這個檢查存在」的全部理由。
它應該在任何輸出之前執行。

## NJ. 一則錯誤訊息的英文半段在句子中間被截斷（程式缺陷）

**狀態：已修（2026-09-07）——補上 `read one`；T262e 釘住。**

```
csv2: ... "-md is one-way and csv2 cannot read it back", which stopped being true when it learned to
```

295 bytes，沒有結尾子句。**同一則訊息的繁中半段是完整的**（「……而在它學會讀回來的那一刻，那句話
就不再為真」）。不是長度限制：`--headers` 不一致那則有 407 bytes 且完整。

**而那被切掉的部分，正是告訴使用者怎麼補救的那一段。**

（我自己在 2026-09-07 稍早的工作中讀過這則訊息至少三次，而沒有注意到它是斷的。）

## NK. 退出碼與錯誤碼沒有契約

**狀態：已記載（2026-09-07）——「每一次失敗都以 1 結束」、三個 `code` 的表、`-get` 也拒絕 `--json` 且會**取代**真正的錯誤、錯誤永遠恰好兩行（`--json` 一個物件），以及把 `--in-place` 的全有全無寫成**承諾**而不是推論。並誠實說明 `invalid-input` 與 `conflicting-options` 的分法跟隨「檢查寫在哪裡」，不是呼叫端能推論的規則。**

- **沒有退出碼契約。** 全書只有「All errors exit non-zero」。回報者測了 30 種失敗，**全部是 1**，
  而那件事沒有被承諾。一句「每一次失敗都以 1 結束」就夠了。
- **沒有 `code` 值的清單。** README 說 `code` 是「stable」而只展示過一個（`not-found`）。要寫一支
  分支的腳本，得靠實驗找出 `invalid-input` 與 `conflicting-options`。
- **`code` 的分法沒有記載，而且看不出規則**：`--physical --json` 與 `--build-index --no-index` 是
  旗標衝突，卻回報 `invalid-input`；`-md --json` 與 `--search-row --search-column` 也是旗標衝突，
  卻回報 `conflicting-options`。
- **`-get` 拒絕 `--json`，這件事不在任何地方**——而它比 `--physical` 那個更要緊，因為那則訊息會
  **取代**真正的錯誤（越界的紀錄號、不存在的欄名）而不是補充它。
- **錯誤的行數保證沒有寫**：純文字錯誤永遠恰好 2 行、JSON 錯誤永遠恰好 1 行——那是腳本最需要的東西。
- **`--in-place` 的「全有全無」保證只以例外的形式存在。** 編輯那一節描述了暫存檔加 rename 的機制、
  然後為 `-append` 開了例外，讀者只能從那個例外**反推**出保證。它在六次失敗測試中都成立，
  **值得被寫成一個承諾，而不是一個推論。**

## NL. `-append` 的描述低估了它自己

**狀態：已修（2026-09-07）——改為「驗證**整個**檔案」，並附上「8000 筆中第 5000 筆有錯會被指名」的實例。**

「仍會讀取既有檔案以驗證最後一筆紀錄」——實測：對一個「第 5000 筆有錯、共 8000 筆」的檔案追加，
會被拒絕並**指名第 5000 筆**。它驗證的是**整個檔案**，不是最後一筆。行為是好的，描述不準確。

---

# 第 91 回合（2026-09-07，`--pretty` 與顯示寬度）

## NM. 「`--pretty`（另一個旗標）」——它不是另一個旗標，而那句是我寫的

**狀態：已修（2026-09-07，文件）——明說兩者是同一個設定的兩種寫法、輸出逐位元相同、共用同一個上限；T263a 釘住。**

```sh
csv2 -r -t -md --pretty          -i d.csv2 > a
csv2 -r -t -md --md-style pretty -i d.csv2 > b
cmp a b        # 逐位元相同
```

兩者共用同一個 `CSV2_PRETTY_MAX_BYTES`，而那個上限的錯誤訊息**即使你給的是 `--md-style pretty`，
也會說 `-md --pretty`**——指名了一個不在使用者指令裡的旗標。

那個括號是我在第 79 回合加的，**它存在的唯一目的就是告訴讀者「這兩個不一樣」**——而那正是一個讀者
在「該用哪一個來避開記憶體上限」時會依賴的句子。

`--md-style` 的表把 `pretty` 列為三個值之一，而散文把 `--pretty` 當成另一件事；於是這一頁把**同一個
設定描述了兩次**，卻從未說出它們是同一個。

## NN. `--pretty` 與 `--md-style` 併用時，**順序決定結果，而且完全靜默**（程式缺陷）

**狀態：已修（2026-09-07）——記住「明確給定的樣式」而不是最終狀態，兩種順序都拒絕。第一版只抓到一種順序，被 T263b 的「兩種順序」斷言抓到；記進 mistakes 第 1 條。T263c 確保一致的組合沒有被連坐。**

```sh
csv2 -r -t -md --pretty --md-style compact -i d.csv2   # |zlib|1.3|
csv2 -r -t -md --md-style compact --pretty -i d.csv2   # | zlib            | 1.3             |
```

**同一組旗標、兩種輸出，rc=0，stderr 零位元組。**

而這支工具在別處**正是**拒絕這種東西的：`--build-index` 與 `--no-index` 被以「兩者互相矛盾」拒絕；
`--md-style aligned` 被拒絕並附上合法值清單。

**一支「在使用者的旗標後面追加一個 style」的包裝腳本，會靜默地把它的輸出翻面。**

## NO. `--pretty` 與 `--md-style` 在沒有 `-md` 時是靜默的無操作（程式缺陷）

**狀態：已修（2026-09-07）——兩者都改為拒絕並指名 `-md`，與 `--filter` 少了 `-contains` 同一個形狀；T263d 釘住。**

```sh
csv2 -r -t          -i d.csv2 > p1
csv2 -r -t --pretty -i d.csv2 > p2
cmp p1 p2      # 相同；rc=0，stderr 零位元組
```

而 `--filter`、`-A`／`-B`／`-C`、`--normalize` 在沒有 `-contains` 時**全部會被拒絕**，README 還特地
寫著「少了它們會這樣說」。**同一種形狀的錯誤，一邊指名、一邊沉默。**

## NP. 兩個記憶體上限的單位不同，而它們在同一張表裡都寫成「16 MiB」

**狀態：已記載（2026-09-07）——表格改成「資料儲存格已跳脫的位元組」與「輸入檔的原始位元組」。**

回報者以五個檔案二分法量出來：

- `CSV2_PRETTY_MAX_BYTES` 數的是**資料儲存格的位元組**（已跳脫、不含標頭）
- `CSV2_MD_MAX_BYTES` 數的是**檔案的原始位元組**

表裡的描述是「maximum material held by `--pretty`」與「maximum Markdown input」——「material」與
「input」是唯一的信號，而那太薄了，薄到不能據以行動。

另外：`CSV2_PRETTY_MAX_BYTES=0` 會被拒絕（低於最小值 1），而 `CSV2_MD_MAX_BYTES=0` 被接受並生效。

## NQ. 超過上限時會發生什麼，完全沒有記載

**狀態：已記載（2026-09-07）——rc=1、stdout **空白**、訊息指名逃生口；T263e 釘住那個空白，因為「錯誤」那一節剛說過檔案中途的錯誤會留下紀錄。**

任務 3 從那一頁上得不到答案。實際行為（rc=1、stdout 空、雙語訊息並指名兩個逃生口）**很好，而且
完全沒有被寫下來**。而在「錯誤」那一節剛剛才說「檔案中途的錯誤不會讓 stdout 保持空白」之後，
讀者**有充分理由害怕這裡也會有部分輸出**，卻沒有任何方法查明。

## NR. 寬度模型只有一句話

**狀態：已記載（2026-09-07）——逐欄 `max(標頭, 最寬儲存格)` 兩側各一空格、量在已跳脫的文字上、以 grapheme cluster 為單位、CJK 算 2、結合字元算 0、ZWJ 序列算一個（並說明那一注押在哪一邊）、`.csv2` 量的是 `<br>` 接起來的字面值，以及「`preserve` 在 CSV 輸入上等同 `compact`」。**

「aligns by DISPLAY width, so the CJK titles count two columns each」就是全部。缺的是：以 grapheme
cluster 為單位、ZWJ 序列算一個、variation selector 會被尊重、結合字元寬度為零、寬度是量在**已跳脫**
的形式上、而 `.csv2` 量的是那個以 `<br>` 接起來的標頭字面值。

**回報者說：少了這些，任務 2 無法推理——「我自己的驗證程式錯了三次」。**

以及那個模型隱含的取捨：以 grapheme cluster 對齊，在支援 ZWJ 的終端機上是對的，在其他終端機上每個
ZWJ emoji 會短兩欄。**使用者需要知道這一注押在哪一邊。**

還缺：每欄寬度的實際規則（`max(標頭, 最寬的儲存格)` 兩側各加一個空格），以及「沒有任何**非
Markdown** 的對齊輸出」——那件事屬於「什麼時候該停止使用它」，而它不在那裡。

---

# 第 92 回合（2026-09-07，定址）—— 第 1 類是**空的**

回報者的判詞：**「定址被記載成一種語法，卻從未說出哪些位址是可以『寫』的、哪些只是會被『印』出來
的。」** 標頭位址（`0`／`0a`／`0b`）與 A1 儲存格（`[C3]`）看起來都像可以打進去的東西。兩者都不是。

## NS. `-rownum` 在定位報告上被靜默忽略，而完全相同的情況在 `--json` 下是硬拒絕（程式缺陷）

**狀態：改判——那個不對稱是真的，而回報者建議的方向是錯的（2026-09-07）。** 我照建議加了拒絕，而 **T15a／T15d 立刻失敗**：T15 的註解明說「`-rownum` 只作用在輸出側：它必須不改變定址」，而它正是以「跑 `-contains -rownum` 並檢查位址沒有移動」來斷言這件事。拒絕那一對，等於刪掉那個不變量被表達的方式——而那個不變量更早、也更承重。程式改動已撤回、理由寫在原處；`--json` 那個拒絕保留（那裡它不只是惰性的），而**不對稱本身被記載下來**。T264c 同時斷言兩半。

**這一項的價值不在缺陷，在於它示範了規則 8 的另一半**：驗證一項回報，不只是確認「它說的現象是真的」，還包括確認「它建議的補救不會踩掉一個更早的決定」。這棵樹的測試替我擋下了那一步。

```sh
csv2 -contains 1. -rownum -i p.csv2          # rc=0，stderr 0 bytes，輸出與不給 -rownum 完全相同
csv2 -contains 1. -rownum --json -i p.csv2   # rc=1
# "-rownum adds a column and --json names fields instead of numbering them, so it would be
#  ignored; --json already carries the record number in its `record` key"
```

**那則拒絕自己的理由——「so it would be ignored」——一字不差地適用於定位報告**，因為那裡的位址本來
就帶著紀錄號。

**這支工具拒絕了無害的那一個，而靜默地接受了完全相同的另一個。** 而靜默的那一支，才是會讓人賠掉
一個下午的那一支。

## NT. 標頭位址在四個入口得到三種品質的錯誤（程式缺陷）

**狀態：已修（2026-09-07）——`--search-row` 現在認得 `0`／`0a`／`0b` 並給出與 `-get`／`--search-cell` 相同的訊息；T264a 一次斷言全部四個入口，因為那個缺陷**就是**它們之間的分歧。**

```sh
csv2 -get 0:1 -i p.csv2                      # 「names a header cell ... no verb can address one」 ✅
csv2 -contains x --search-cell 0a:1 -i ...   # 同一則好訊息                                        ✅
csv2 -contains x --search-row 0a -i ...      # 「--search-row: not a number: 0a」                  ✖
csv2 -contains x --search-row 0 -i ...       # 「--search-row 0: a count must be at least 1」      ✖
```

**`--search-row 0` 正是一個讀者在讀完「標頭位址是 `0`」那句話之後，下一步會打的東西**——而它是唯一
一條用**錯誤的概念**回答他的路：一個列位址不是一個計數。

**那則好訊息已經在執行檔裡了，只是兩個呼叫點到不了它。**

## NU. 「Header addresses are `0` for `.csv`, and `0a`/`0b` for `.csv2`」——這一頁最誤導的一句

**狀態：已修（2026-09-07）——改寫成「標頭位址是 csv2 會**印出來**的東西，絕不是你可以**寫進去**的東西」，並列出三種會被拒絕的寫法與 `--include-headers` 這個出路。**

兩種讀法：(a)「這些是你可以寫的位址」、(b)「這些是報告會印出來的東西」。**只有 (b) 為真。**

而它的**位置**把讀者推向 (a)：它夾在 `-get r:c` 與 `--search-cell R:C` 之間，而那兩個都是**收位址**的。

修法用這支工具自己的話：在後面加上「**而沒有任何動詞可以定址它**」。

## NV. 七件沒有記載的事

**狀態：已記載（2026-09-07）——`--a1` 的列號是「標頭列數 ＋ 紀錄號」而非實體行號（T264e 釘住）、沒有動詞能定址標頭、`--a1` 需要 `-contains`、`-rownum` 的作用範圍、`-rownum --json` 的拒絕、欄名只能來自英文列，以及報告的欄名欄位跟隨 `--en`／`--zh`。**

1. **`--a1` 的列號怎麼算**：是「標頭列數 ＋ 紀錄號」，**不是實體行號**。唯一的範例 `2:2 [B3]` 沒有
   附檔案，因此兩種讀法都成立。**這是價值最高的那個遺漏**——`--a1` 存在的理由就是「交給一個開著
   試算表的人」，而那個數字會因為一個引號內的換行而差掉。
2. **沒有任何動詞可以定址標頭儲存格。**
3. **`--a1` 需要 `-contains`，且與 `--filter`、`-md`、`--json` 併用會被拒絕。**
4. **`-rownum` 只影響「紀錄輸出」**，在定位報告上是無操作。
5. **`-rownum` 與 `--json` 完全不能併用**（硬拒絕，未記載）。
6. **「欄名只能來自英文那一列」寫在錯的章節**——它在「搜尋範圍」底下，而需要它的讀者在「編輯」那一節
   讀到「任何接受欄號的地方都可以用欄名」。
7. **定位報告的欄名欄位跟隨 `--en`／`--zh`**，而且即使那個命中來自 `0b:`（中文列），預設仍是英文。

## NW. 「they combine freely with `--filter`、`-A`/`-B`/`-C`、`-rownum` 與 `--json`」——逐項為真、整句誤導

**狀態：已修（2026-09-07）——改成「它們**各自**都能與那四個配對，但那四個**彼此之間**並非都相容」，並舉出 `-rownum --json`。**

逐項檢查是對的（每一個範圍選項確實都能與其中每一個配對）。但一個四項的「可以自由併用」清單，自然
的讀法是**那些項目彼此也可以同時出現**——而 `-rownum --json` 是一個 rc=1 的硬拒絕。**兩種讀法，
一種是假的。**

---

# 第 93 回合（2026-09-07，`--value-file`／`--value-stdin`）—— 第 1 類是空的

「關於這兩個旗標的每一句肯定陳述都為真，而我逐句試著弄壞它。」——回報者。

## NX. `--value-file (path)` 是一個沒有被替換的佔位符（程式缺陷）

**狀態：已修（2026-09-07）——訊息現在指名路徑**與**成因，四種成因各有自己的說法（沒有這個檔案／那是一個目錄／不是一般檔案）；空的一般檔案仍然被接受。**

```sh
csv2 -update 1:3 --value-file /nowhere/NOSUCH.bin -i base.csv --in-place
# csv2: --value-file (path): cannot read the value file
```

**字面的 `(path)` 印在路徑該出現的位置**，兩種語言都是，涵蓋四種不同成因（檔案不存在、目錄、`-`、
`/dev/null`）。回報者以位元組層級確認過。

這支工具其他每一則同類訊息都指名它的主體——`cannot open input file: nosuch.csv`、
`the file has 2 records`、`naming 5000`。**一支對許多值檔案跑 `-update` 的腳本，拿到的是一個說不出
「哪一個失敗了」的錯誤。**

**同一則訊息還誤述了自己的成因**：`/dev/null` 被以「無法讀取」拒絕，而同樣大小的空**一般**檔案是
被接受的（rc=0，那一格被清空）。所以那個拒絕是關於「不是一般檔案」，不是關於「讀不到」或「是空的」。

## NY. 「為什麼這兩個旗標存在」是那一頁最需要說、而沒有說的一件事

**狀態：已記載（2026-09-07）——連同「那個截斷發生在 csv2 之前」這個事實，以及兩個可直接貼的範例。**

字面值那條路**會在 NUL 處靜默截斷**：

```sh
csv2 -update 1:3 "$(printf 'AA\000BB')" -i n.csv --in-place   # rc=0，存進去的是 AA
csv2 -update 1:3 --value-file v.bin     -i f.csv --in-place   # 存進去的是 AA\0BB
```

**這不是 csv2 的缺陷**：POSIX 的 `execve` 參數是 NUL 結尾的 C 字串，shell 根本傳不進一個 NUL，
csv2 收到的就是 `AA`。而那正是這兩個旗標存在的**全部理由**，加上「非 UTF-8 的字面值會被直接拒絕」。

回報者把它歸在「文件沒寫」而不是第 4 類——**那個歸類是對的**，而它是這一輪最有價值的一項。

## NZ. 十件沒有記載的事，其中第一件最大

**狀態：已記載（2026-09-07）——`-get` 的單一 LF 與一行 `cmp` 的往返檢查、`--json` 對非 UTF-8 的拒絕、`<non-UTF-8:>` 的呈現、`--value-file -` 不是 stdin、互斥與不可重複、`--in-place` 與 `--value-stdin` 可以併用。**

1. **怎麼把位元組讀回來。** 這兩個旗標賣的是位元組保真度，而那一頁從未說怎麼驗證它。
   `-get` 會加上**恰好一個** LF 結尾——每一次往返都需要知道這件事，而它不在任何地方。
2. **`--json` 會拒絕一個含有非 UTF-8 位元組的檔案。** README 把 `--value-file` 記載成「把任意位元組
   放進儲存格」的方法，把 `--json` 記載成機器可讀的輸出——**而從未說用了前者會讓後者失敗**。
   兩節必須互相引用而沒有。
3. **`<non-UTF-8: 58 ff 59>` 這個呈現**（定位報告與 `--dry-run` 都會用），沒有規格。
4. **跳脫清單不完整**：報告記載了 `\t`、`\n`、`\\`，而它也會輸出 `\x00` 與上面那個形式。
5. **沒有任何 `--value-stdin` 的範例**——四次提及、零個範例。
6. **在這兩個旗標之下，`-update` 只收一個參數**（位址）。
7. **空的輸入會被接受**，把那一格清空、rc=0、沒有任何輸出。
8. **`--value-file -` 不是 stdin。** 每一個讀者都會試。
9. **不可重複**，而且**兩者互斥**。
10. **`--in-place` 可以與 `--value-stdin` 併用**——那條「拒絕 stdin」講的是 `-si`（輸入），
    而一個自然的讀法會以為它禁止了這個旗標最自然的用法。

---

# 第 94 回合（2026-09-08，保護與復原）—— 第 1 類連續第三輪是空的

回報者逐一測了它依賴的每一項宣稱——原子 rename、失敗後逐位元不變、備份不覆寫的拒絕、`-append`
的整檔驗證、append/append 的並行例外、索引過期規則、`.bak` 讀成逐行模式、串流部分輸出的危險、
欄數不齊的拒絕、`--truncate-partial` 與 `-append` 的互斥——**每一項都與寫的完全一致**。

## OA. 同一支程式裡的兩則警告有兩種形狀，其中一則洩漏了稽核記錄的格式（程式缺陷）

**狀態：已修（2026-09-08）——WARN 到 stderr 改用 `csv2: <文字>`，與 `-add-column` 一致；`-debug` 的追蹤保留時間戳與等級（那裡順序與間隔本身就是資訊）。T266c／T266d 同時釘住兩半。**

```
csv2: 2026-09-08T01:14:06.597+08:00 WARN  --truncate-partial discarded 12 bytes: ...
csv2: -add-column 2 note: no Chinese title given, row 2 left empty; ...
```

第一則是在一次**從未給過 `-log`** 的執行上印出來的——ISO 時間戳與 `WARN` 等級，那是稽核記錄**檔案**
的格式。起因在 `Logger.log()`：它建**一個**字串，同時寫進檔案與 stderr。

在檔案裡，時間戳與等級**就是那筆紀錄**；在 stderr 上，它們是雜訊，而且與這支工具印出的其他每一則
訊息都不一樣。

## OB. 一次失敗的編輯留下 `.bak`，而那永久封死了那個路徑上未來的每一次 `--backup`（程式缺陷）

**狀態：已修（2026-09-08）——中止時連同備份一起移除。T266a 斷言**四種**狀態（失敗不留／之後可用／成功保留／重複仍拒），因為任一方向的修法都會通過其中兩種；T266b 另外斷言備份保存的是**編輯前**的位元組。**

```sh
csv2 -update 99:1 X -i f.csv --in-place --backup   # rc=1，而 f.csv.bak 被建立了
csv2 -update 1:1  X -i f.csv --in-place --backup   # csv2: backup path already exists ...
```

備份寫在編輯**之前**，那是必然的——它存在的理由就是保存「那次編輯即將取代的位元組」。但一次失敗的
編輯會讓輸入**逐位元不變**，於是那個 `.bak` 保存的是一份沒有東西動過的檔案；而 `makeInPlaceBackup`
拒絕覆寫既有的備份。

**於是 `--backup` 每個檔案只能用一次，而一次失敗就把那唯一的一次燒掉了。**

這與 T28d 是同一個形狀——那個案例斷言「一次失敗的就地編輯不會在輸入旁邊留下暫存檔」。**一個過期的
`.bak` 就是那份殘骸換了個名字。**

## OC. 我第 84 回合寫的 `-debug` 範例，是一份從未發生過的轉錄

**狀態：已修（2026-09-08）——改成真實輸出，並註明「它活下來是因為那個案例用子字串比對」。**

我寫的是裸的 `index hit` 與 `metrics: ...`。真實輸出是：

```
csv2: 2026-09-08T01:17:16.211+08:00 DEBUG index hit: record 500 via grid point 257 at byte 6297
```

**那與我在第 88 回合修掉的 MV 是同一個毛病**（一段假的主控台轉錄），而它活下來是因為釘住它的
T256e 用的是**子字串**比對——`== *'index hit'*` 對兩種形式都成立。

**一個用子字串比對輸出格式的案例，釘住的是內容、不是格式。**

## OD. 八件沒有記載的事

**狀態：已記載（2026-09-08）——破損的定義（未關閉的引號）、`--truncate-partial` 在讀取上也有效、rc=0 之下的警告形狀、`--backup` 每路徑一次、`-count` 不驗欄寬但會驗引號。T266e 釘住「欄位太少的最後一筆會被拒絕」這個界線。**

1. **「結尾不完整的紀錄」從未被定義。** 它指的是「檔案結束在一個未關閉的引號裡」。回報者的四種
   候選形狀有三種是**另一種**錯誤。
2. **`--truncate-partial` 在一般讀取上也有效**，不只在重寫時——那正是一個手上拿著破損檔案的人要的。
3. **rc=0 之下的 stderr 警告從未被提及。** 錯誤那一節寫著「永遠恰好兩行」與「每一次失敗都以 1 結束」；
   警告兩者皆非，而那一頁點名了三則警告，卻從未說它們長什麼樣、也沒說 stderr 在 rc=0 時可能非空。
4. **一次 SIGKILL 會永久留下 `.NAME.csv2tmp.PID`**——權限 0600、你的資料的一份部分複本，沒有東西會
   清掉它。那一頁只把私有暫存檔當成機制提過，從未當成殘骸。
5. **一次失敗的 `-o` 編輯不會產生輸出檔。** 行為是好的；那個保證只針對輸入被寫出來過。
6. **`-count` 不驗證欄寬。** 20000 筆中第 15000 筆不齊時，`-count` 在 rc=0 之下印出 `20000`，
   而 `-r` 拒絕那個檔案。**一個「沒有別的東西讀得了的檔案」的自信計數**，正是這一頁其餘部分費力
   要防的那一類。（它**會**檢查引號封閉。）
7. **沒有辦法驗證那個當機保證。** 那一頁承諾「不存在輸入寫到一半的狀態」，卻沒有給任何配方。
8. **`--backup` 每個路徑只能用一次**，而那一頁從未說要輪替 `.bak`。

## OE. 並行那個例外，範圍寫錯了

**狀態：已修（2026-09-08，文件）——那張表的一列改成「安全的並行寫入，**append 對 append 除外**」。未加測試：那是一場競賽，而一個有時通過的案例比沒有案例更糟。**

「兩個並行的 `-append --in-place` 是例外：兩筆紀錄都會完整落地」——**為真，但只限 append 對 append**。
...
# 第 96 回合（2026-09-08，install.zsh）—— 第 1 類是空的

「八次安裝呼叫，**零次失敗、零則錯誤訊息**。」那一句記載旗標的話是準確的——**它只是幾乎是那裡唯一
的一句話。**

## OK. 安裝器回報了一個它沒有做過的驗證，而那個宣稱是假的（程式缺陷）

**狀態：已修（2026-09-08）——「非登入找得到」那一支現在也跑一次登入 shell，若解析到不同的檔案，就改為回報「非登入 shell；而**登入** shell 會先找到 X」並指名那個競爭者。實測在本機（`/usr/local/bin/csv2` 存在）上如實回報。**

```
verified: a fresh zsh runs …/deeper/bin/csv2, and it is the file just installed
  reachable from: every shell, scripts over ssh included
```

而一個**登入** shell 跑的是 `/usr/local/bin/csv2`，不是剛裝的那個檔案。

那個探測是一個**階梯**：非登入 → 登入 → 登入互動，只要最受限的那一層找得到，就結論「每一種 shell」。
**那個推理在「登入 shell 只會增加 PATH」時成立**——而 macOS 上不成立：`path_helper` 由
`/etc/zprofile` 在登入 shell 執行，它會**重排** PATH。

**修法就寫在這份 README 自己的第 35–39 行**，而三行之上的那個檢查沒有用到它。

`reachable from: every shell` 對**可達性**為真、對**同一性**為假——而它旁邊那一行承諾的正是同一性：
「and it is the file just installed」。

**條件要誠實說**：這需要登入 PATH 上有另一個 csv2。回報者的機器上本來就有一個（9 月 2 日的
`/usr/local/bin/csv2`，不是那個回合放的）。**那不是什麼罕見的配置——那正是「你裝第二次」時會有的
東西，而 `--prefix` 與 `--dir` 存在的理由就是讓你裝第二次。**

**這是同一個教訓的第二層。** 2026-08-25 這支腳本已經學過一次：`zsh -c` 會繼承呼叫者的 PATH，於是
「csv2 在我已經有的 PATH 上嗎」被回報成「每一種 shell 都找得到」。那次的修法是「從零開始一個 shell」。
這一次是：**從零開始，但只從一種零開始。**

## OL. `--uninstall` 在 `$HOME` 留下殘骸（程式缺陷，輕微）

**狀態：已記載（2026-09-08），未改動行為。** 移除一個「使用者可能在別處也依賴」的啟動檔，比留下它更危險：`~/.zshenv` 是共用的，而這支安裝器只擁有它寫下的那個區塊。文件現在明說它會建立那個檔案、以及 `--uninstall` 不會移除它。

它移除自己標記的區塊，但不移除它**自己建立的** `~/.zshenv`——留下一個原本不存在的檔案，而且每一次
安裝／移除循環會多累積一個空行（實測：不存在 → 2 bytes → 3 bytes）。

## OM. 八件沒有記載的事

**狀態：已記載（2026-09-08）——旗標改成一張表並說明可組合、預設位置與它自身的警告的矛盾、會寫 `~/.zshenv`、`--uninstall` 單獨使用只看預設位置、裝到別處會留下孤兒，以及「rc=0 不代表可以用名字執行」。**

1. **預設安裝位置從未被寫出來。** 只有 `--dry-run` 找得到它。預設是 `/opt/homebrew/bin`——正是
   `README.md:35` 警告「經 ssh 的腳本不要用」的那個目錄。**這一頁勸退它自己的預設值，而沒有說那是
   預設值。**
2. **安裝器會編輯一個 shell 啟動檔，而兩份 README 都沒說。** 它寫的是 `~/.zshenv`，不存在時建立。
   `--no-rc` 被列出來但沒有任何說明，因此讀者看得出「有某種 rc 相關的事發生」，卻不知道是什麼、
   在哪裡、怎麼還原。
3. **`--uninstall` 單獨使用時只檢查預設位置**，其餘什麼也不移除、rc=0。
4. **`--uninstall` 可以與 `--prefix`／`--dir` 組合**——那是移除非預設安裝的唯一方法。
5. **裝到別處會把先前的執行檔留在磁碟上**，並靜默替換 PATH 區塊，讓前一個變成孤兒。
6. **rc=0 不代表「csv2 現在可以用名字執行」。** 在 `--no-rc` 之下明確地不代表。
7. **`--uninstall` 會留下 `~/.zshenv`**（見 OL）。
8. **`--prefix`／`--dir` 會建立不存在的目錄**（`mkdir -p`，可以任意深）。

## ON. 四處不清楚

**狀態：已修（2026-09-08）——旗標表取代了那個扁平清單；`--prefix`／`--dir` 明說會建立目錄；而「登入／非登入」那一項隨 OK 一起修好，訊息現在指名它探測的是哪一種。**

1. **「ask a fresh shell」是登入還是非登入。** 安裝器指的是非登入；而一個打開終端機的使用者拿到的是
   登入。**兩者不一致——那正是 OK 的來源。**
2. **五個旗標是一個扁平的逗號清單**，那邀請讀者把它們讀成互斥的選項。其中四個彼此可以組合，而
   `--uninstall` 若不能組合就是誤導。
...
**狀態：已修（2026-09-08），由 T280a–T280d 釘住。** 四個案例覆蓋兩半：欄數相符的 `#` 行是資料、以 `#` 開頭的標頭是合法欄名、欄數不對的 `#` 行拿到的是不提 `#` 的欄數錯誤，以及分隔列只有在它是該行唯一欄位時才被拒絕。T272a 缺的正是後面那三個。
...
**狀態：已修（2026-09-08），由 T287a 釘住。** 那個案例檢查的是**那一對**文件的性質——兩份都要說 base64、兩份都不得再宣稱空儲存格分辨得出來——因為單看任一頁都讀得通，而那正是這個缺陷能存在的原因。
...
**狀態：已修（2026-09-08），由 T281a／T281b 釘住。** 修法不是把「三」改成「五」——那是同一個錯換個大小——而是說明這個集合**不是封閉的**並列出已見過的五個。T281b 用**正向**斷言（那兩頁必須說出它是開放的），因為第一版禁止舊字串時抓到了自己：修正後的段落在它自己的歷史註記裡引用了那句話。
...
**狀態：已修（2026-09-08），由 T282a 釘住。** 三處都改寫成那個窄的、為真的事實；「何時該停用」那張表底下移除該列的**理由**也一併更正。
...
**狀態：已修（2026-09-08）。** 第 1、3 兩項由 T286c 釘住；第 2、4 兩項是純敘述，已寫進 `--json` 那一節。
...
**狀態：已修（2026-09-08）。** 註記改成過去式，說出那一段**先前**是什麼。
...
**狀態：已修（2026-09-08），由 T295a 釘住。** 那個案例把「兩頁一致」從**一對**升級成一個**類別**：機器介面上每一個承重的詞都必須同時出現在兩份文件，而且一個從某一頁移除的承諾不得在另一頁存活。T287a 抓不到 PR，因為它釘的是加密那一對。
...
**狀態：已修（2026-09-08），由 T294a／T294b 釘住。** 兩個宣稱都補上了前提；T294a 斷言小檔上是 0 而不是 141，T294b 要求兩份文件都寫著那個前提——因為只斷言行為，擋不住「未來某次編輯又把限定詞拿掉」。
...
**狀態：已修（2026-09-08），由 T291a 釘住。** 限制表新增了「精確的整格搜尋」那一列，含一份實測過的 jq 做法；T291a 釘住的是「限定欄位仍然是子字串比對」這個事實。
...
**狀態：已修（2026-09-08），由 T288a／T288b 釘住。**
...
**狀態：已修（2026-09-08）。** 第 1 項由 T292a 釘住；第 2 項是定義，已寫進 `-count` 那一段。
