...
| 2026-09-08 | **交錯量測第 2 輪** | 量尺是那支腳本自己 | 我在迴圈**正在執行** `measure.zsh` 的時候改了它。那一輪的兩個寫入列回報 `0`，而 `0` 在一張時間表裡看起來像「這一列沒跑」，不像「量尺在跑到一半時被換掉了」 |
| 2026-09-09 | **guest 有沒有 zstd** | 量尺是一份**描述那台機器的檔案**——而且是上游自帶的範例 | 那台機器**當時開著**。`command -v zstd` 一行就能問。我讀了 24 行 defconfig，得到相反的答案，然後把它寫進 tag、release notes、兩份 README，還據此請另一個 session 改他們的設定 |
...
2026-09-09。「guest 的映像裡有沒有 zstd？」——這個問題有兩種問法。我選了讀
`sos/buildroot/configs/aarch64_efi_defconfig`，看到 `grep -c ZSTD` 是 0，結論是「沒有」。

那個檔案是 **Buildroot 上游自帶的範例**（`git log` 顯示最後一次改動是上游的 commit）。這個專案實際
餵給 buildroot 的是 `sos/linux_kernal_vm_interactive/build/buildroot-multissh.config`，它第 297 行是
`BR2_PACKAGE_ZSTD=y`，而第 260 行是一段**解釋為什麼需要它**的註解。guest 裡有 `/usr/bin/zstd`，
205848 bytes。

**這一條與前十七次的差別，在於它連測試都沒有——只有一次閱讀。** 但形狀相同：我碰的不是被問的那個
東西。而代價比前幾次大，因為那個假前提沒有停在我的推論裡：它進了 `git tag -a v0.1.0` 的訊息（**改
不了**）、release notes（已更正）、兩份 README（已更正），以及一個**送給另一個 session、請他們在重建
裡加一行他們早就加過的設定**的請求。對方得進 guest 裡實際 `ls` 一次來反駁我。

**現行防範：** 一個「關於某台機器」的問題，只有那台機器能回答。設定檔回答的是「有人打算讓它是什麼
樣子」，而那與「它現在是什麼樣子」是兩件事——中間隔著這份設定有沒有被用到、建置有沒有成功、以及
之後有沒有人改過它。那台機器開著的時候，問它；沒開的時候，說「我不知道」，不要用檔案代替。

同一天同一個形狀還小規模地出現了第二次：我估 rootfs 成本時用的是**手邊 Debian 的** zstd（1 MiB），
而 guest 裡實際是 205 KB。

### 第十七次：量尺是那支腳本自己

2026-09-08，第 98 回合。一個交錯執行的 benchmark 正在跑「SSD 對 sparse image」，而我在它執行到
一半時去修 `measure.zsh`——那正是它每一輪都在呼叫的那支腳本。第 2 輪的兩個寫入列因此回報 `0`。

**這一條之所以屬於第 1 類，是因為量尺與被測物之間的界線在這裡消失了。** 前十六次是「工具沒有那個
能力」或「fixture 弄丟了那個屬性」；這一次是**我自己在量測期間改變了量尺**。而它的症狀與前面每一次
相同：一個看起來合理的結果（`0`），在一張全是數字的表裡讀起來像「那一列沒跑」。

它沒有造成錯誤的結論，只因為我**看了那一列**並且問了「為什麼是 0」。若那兩格填的是任何一個似真的
數字，它會直接進到 README 裡。

**現行防範：** 一次量測開始之後，在它結束之前，不要碰它會讀到的任何東西——腳本、輸入、被測的執行檔。
這條與 skill 的「關口 2」是同一件事的另一半：那裡說的是「量測方法」（交錯、取最小值、10 輪），
這裡說的是「量測期間」。跑 benchmark 時要做的別的事，等它結束。
